...
impl AGC {
    ...
    pub fn step(&mut self, x: f32) -> f32 {
        self.peak *= if abs(x) > self.peak { self.boost } else { self.decay };
        if abs(x) > (self.peak * self.thresh) { 0.0 } else { 100.0 * x / (2.0 * self.peak) }
    }
